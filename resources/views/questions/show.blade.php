...
@section('content')
                ...
                @auth
                    @can('update', $question)
                        <div class="question-actions">
                            <a href="{{ route('questions.edit', $question) }}" class="btn btn-outline btn-sm">Izmeni</a>
                            <form action="{{ route('questions.destroy', $question) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-outline btn-sm delete-btn">Obriši</button>
                            </form>
                        </div>
                    @endcan
                @endauth
    ...
    <div class="answers-section" id="answers">
        <h2 class="answers-title">{{ $question->answers->count() }} Odgovora</h2>
        
        @foreach ($question->answers as $answer)
            <div class="answer-card {{ $answer->is_solution ? 'solution' : '' }}">
                @if ($answer->is_solution)
                    <div class="solution-badge">Prihvaćeno rešenje</div>
                @endif
                ...
                <div class="answer-header">
                    ...
                    @auth
                        <div class="answer-actions">
                            @can('update', $answer)
                                <a href="{{ route('answers.edit', $answer) }}" class="btn btn-outline btn-sm">Izmeni</a>
                            ...
                            @can('markSolution', $question)
                                @if (!$answer->is_solution && $question->status !== 'resolved')
                                    <form action="{{ route('answers.solution', $answer) }}" method="POST" class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-outline btn-sm">Označi kao rešenje</button>
                                    </form>
                                @endif
                            @endcan
                        </div>
        ...
        @auth
            <div class="form-container">
                <h3 class="form-title">Vaš odgovor</h3>
                ...
                <form action="{{ route('questions.answers.store', $question) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <label for="content" class="form-label">Odgovor</label>
        ...
        @else
            <div class="login-prompt">
                <p>Morate biti prijavljeni da biste odgovorili na ovo pitanje.</p>
